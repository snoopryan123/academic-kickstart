...
###################################
### NFL Draft Performance Value ###
###################################

### get data
filename_nfldraftperfvalue = "data/df_NFLDraftPerfValue.csv"
df_draft = read_csv(filename_nfldraftperfvalue, show_col_types = F)
df_draft = df_draft %>% group_by(player_id) %>% slice_head() %>% ungroup() %>% select(-player_cap_proportion)

### scatterplot of value versus draft pick
scatterplot_value = 
  df_draft %>%
  # filter(draft_pos <= 161) %>%
  ggplot(aes(x=draft_pos, y=firstContractPerformanceValue)) +
  ylab("First contract performance value") +
  xlab("Draft pick") +
  scale_x_continuous(breaks=seq(1,32*9,by=32)) +
  geom_point()
# scatterplot_value
ggsave("plots/plot_NFLDraftValueVsPick.png", width=9, height=6)
...
### plot spline regressions
plot_draft_splines = 
  df_draft %>%
  mutate(
    v_spline1 = predict(draft_model1, .),
    v_spline2 = predict(draft_model2, .),
  ) %>%
  # filter(draft_pos <= 161) %>%
  ggplot(aes(x=draft_pos)) +
  geom_point(aes(y=firstContractPerformanceValue)) +
  geom_line(aes(y=v_spline1), color="firebrick", linewidth=3) + 
  ylab("First contract performance value") +
  xlab("Draft pick") +
  scale_x_continuous(breaks=seq(1,32*9,by=32)) 
# scatterplot_value
ggsave("plots/plot_NFLDraftValueSplines1.png", width=9, height=6)
...
plot_draft_splines = 
  ...
  ggplot(aes(x=draft_pos)) +
  geom_point(aes(y=firstContractPerformanceValue)) +
  geom_line(aes(y=v_spline2), color="dodgerblue2", linewidth=3) + 
  ylab("First contract performance value") +
  xlab("Draft pick") +
  scale_x_continuous(breaks=seq(1,32*9,by=32)) 
# scatterplot_value
ggsave("plots/plot_NFLDraftValueSplines2.png", width=9, height=6)
