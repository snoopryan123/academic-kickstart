...
### full dataset (for Ryan's local machine)
data_full_0 = read_csv("~/Desktop/RESEARCH/x_pts_nfl/comparison_v110/data7b.csv")

# ### full dataset
# ### IMPORTANT: download the dataset `data7b.csv` from https://upenn.box.com/s/f76spk9ismcjt4y4jebaiv7m4woncvkh
...
data_full_0A = data_full_0 %>% mutate(
...
data_full_A = data_full_AA %>%
  drop_na(pass_or_rush)

### datasets for decision making model fitting
omega = 0.15
...
tail(
  all_fourth_downs %>% select(
    posteam, season, posteam_coach, decision_actual,label_decision_actual,
