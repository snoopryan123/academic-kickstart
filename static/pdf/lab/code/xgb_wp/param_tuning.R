...
# WP = if (is.na(args[3])) FALSE else as.logical(args[3]) ### if TRUE then tune Win Probability models, else tune Expected Points models, default is EP
TUNE_ALL_6_PARAMS_AT_ONCE = FALSE
...
print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
print(paste0("CATALYTIC = ", CATALYTIC, " and j = ", j, " and WP = ", WP))
print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
...
if (CATALYTIC) {
  ...
  if (!WP) { # EP
    ...
    # nrow_grid_1 = 100 # number of combos of (max_depth, min_child_weight) to tune
    # nrow_grid_2 = 38 # number of combos of (subsample, colsample_bytree) to tune, nrow(grid_2) == 2*nrow_grid_2 + 10
    # nrow_grid_3 = 100 # number of combos of (eta, gamma) to tune
    # nrow_grid_1 = 40 # number of combos of (max_depth, min_child_weight) to tune
    # nrow_grid_2 = 15 # number of combos of (subsample, colsample_bytree) to tune, nrow(grid_2) == 2*nrow_grid_2 + 10
    # nrow_grid_3 = 40 # number of combos of (eta, gamma) to tune
  } else { # WP
    TUNE_ALL_6_PARAMS_AT_ONCE = TRUE
    nrow_grid_full = 250
  }
...
##################################################################
if (!CATALYTIC) { # CATALYTIC = FALSE
  if (!WP) { ### EP model tuning
    model_names_list <- list()
  } else { ### WP model tuning
    model_names_list <- list(xgb_wp_110_6_model_name)
  }
} else { # CATALYTIC = TRUE
  if (!WP) { ### expected points model tuning
    ...
    stop("FIXME: catalytic functionality not yet live for Win Prob models")
  }
}
##################################################################
...
xgb_monotonicities <- if (xgb_is_Regression | xgb_is_BoundedRegression | WP) get(paste0(model_name, "_monotonicities"))  
xgb_fit_on_110_data = str_detect(model_name, "110")
num_features = length(xgb_features)

if (WP) {
  xgb_MINmaxdepth = case_when(num_features <= 2 ~ 1, 3 <= num_features & num_features <= 5 ~ num_features - 2, 6 <= num_features ~ 4)
  xgb_MAXmaxdepth = ifelse(num_features > 8, 8, num_features) 
  mtry_range = c( ifelse(round(num_features*0.25) < 1, 1, round(num_features*0.25)), num_features )
} else { ### EP
  #FIXME
...
evaluate_param_combo <- function(params) {
  ...
  if (!WP) { ### expected points model tuning
    VAL_SET_FORTUNING = TRAIN_SET %>% filter(epoch %in% VAL_EPOCHS) 
    TRAIN_SET_FORTUNING = TRAIN_SET %>% filter(!(epoch %in% VAL_EPOCHS)) 
  } else { ### win probability model tuning
    VAL_SET_FORTUNING = TRAIN_SET %>% filter(game_id %in% VAL_GAMES) 
    TRAIN_SET_FORTUNING = TRAIN_SET %>% filter(!(game_id %in% VAL_GAMES)) 
  }
...
list.save(tuning_results, paste0("", paste0(model_name, if (CATALYTIC) "_catalytic" else ""  ), ".yaml"))
